...
LOG_MODULE_REGISTER(tsl25721, CONFIG_SENSOR_LOG_LEVEL);
				 ...
				 enum sensor_channel chan)
{
...
#ifdef CONFIG_TSL25721_ENABLE_ALS
	...
	if (chan == SENSOR_CHAN_ALL || chan == SENSOR_CHAN_PROX) {
		ret = vcnl4040_read(dev, TSL25721_REG_PS_DATA,
				    &data->proximity);
		if (ret < 0) {
	...
	if (chan == SENSOR_CHAN_ALL || chan == SENSOR_CHAN_LIGHT) {
		ret = vcnl4040_read(dev, TSL25721_REG_ALS_DATA,
				    &data->light);
		if (ret < 0) {
...
{
	const struct vcnl4040_config *config = dev->config;
	uint16_t value[TSL25721_RW_REG_COUNT] = { 0 };
	uint8_t reg;
	int ret = 0;
	...
	value[TSL25721_REG_ALS_CONF] = config->als_it << TSL25721_ALS_IT_POS;
	/* Clear ALS shutdown */
	value[TSL25721_REG_ALS_CONF] &= ~TSL25721_ALS_SD_MASK;

	/*
	 * scale the lux depending on the value of the integration time
	 * see page 8 of the TSL25721 application note:
	 * https://www.vishay.com/docs/84307/designingvcnl4040.pdf
	 */
	switch (config->als_it) {
	case TSL25721_AMBIENT_INTEGRATION_TIME_80MS:
		data->sensitivity = 0.12;
		break;
	case TSL25721_AMBIENT_INTEGRATION_TIME_160MS:
		data->sensitivity = 0.06;
		break;
	case TSL25721_AMBIENT_INTEGRATION_TIME_320MS:
		data->sensitivity = 0.03;
		break;
	case TSL25721_AMBIENT_INTEGRATION_TIME_640MS:
...
#else
	value[TSL25721_REG_ALS_CONF] = TSL25721_ALS_SD_MASK; /* default */
#endif

	/* Set PS_HD */
	value[TSL25721_REG_PS_CONF] = TSL25721_PS_HD_MASK;
	/* Set duty cycle */
	value[TSL25721_REG_PS_CONF] |= config->led_dc << TSL25721_PS_DUTY_POS;
	/* Set integration time */
	value[TSL25721_REG_PS_CONF] |= config->proxy_it << TSL25721_PS_IT_POS;
	/* Clear proximity shutdown */
	value[TSL25721_REG_PS_CONF] &= ~TSL25721_PS_SD_MASK;

	/* Set LED current */
	value[TSL25721_REG_PS_MS] = config->led_i << TSL25721_LED_I_POS;
			      ...
			      enum pm_device_action action)
{
	...
	ret = vcnl4040_read(dev, TSL25721_REG_PS_CONF, &ps_conf);
	if (ret < 0)
		return ret;
#ifdef CONFIG_TSL25721_ENABLE_ALS
	uint16_t als_conf;

	ret = vcnl4040_read(dev, TSL25721_REG_ALS_CONF, &als_conf);
	if (ret < 0)
		return ret;
#endif
	switch (action) {
	case PM_DEVICE_ACTION_RESUME:
		/* Clear proximity shutdown */
		ps_conf &= ~TSL25721_PS_SD_MASK;

		ret = vcnl4040_write(dev, TSL25721_REG_PS_CONF,
					ps_conf);
		if (ret < 0)
			return ret;
#ifdef CONFIG_TSL25721_ENABLE_ALS
		/* Clear als shutdown */
		als_conf &= ~TSL25721_ALS_SD_MASK;

		ret = vcnl4040_write(dev, TSL25721_REG_ALS_CONF,
					als_conf);
		if (ret < 0)
			return ret;
#endif
...
static int vcnl4040_init(const struct device *dev)
{
	...
	/* Check device id */
	if (vcnl4040_read(dev, TSL25721_REG_DEVICE_ID, &id)) {
		LOG_ERR("Could not read device id");
		return -EIO;
	}

	if (id != TSL25721_DEFAULT_ID) {
...
DT_INST_FOREACH_STATUS_OKAY(TSL25721_DEFINE)
